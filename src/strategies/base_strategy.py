...
class BaseStrategy(ABC):
    ...
    def calculate_returns(self, positions=None, price_column='Close'):
        ...
        print(f"Calculating returns using positions: {positions.values}")
        print(f"Position counts: Buy: {(positions == 1).sum()}, Sell: {(positions == -1).sum()}, No position: {(positions == 0).sum()}")

        # Calculate returns
        prices = self.data[price_column]
        print(f"Using price column: {price_column}")
        print(f"First few prices: {prices.head()}")
        print(f"Last few prices: {prices.tail()}")

        # Check if prices and positions have the same index
        if not prices.index.equals(positions.index):
            print("Warning: Prices and positions have different indices!")
            print(f"Prices index: {prices.index}")
            print(f"Positions index: {positions.index}")
            # Align indices
            ...
            if len(common_index) > 0:
                ...
                print("Error: No common indices between prices and positions!")
                return pd.Series(0, index=prices.index)

        self.returns = PerformanceMetrics.calculate_returns(prices, positions)
        print(f"Calculated returns: {self.returns.head()}")
        print(f"Returns stats: min={self.returns.min()}, max={self.returns.max()}, mean={self.returns.mean()}")

        # Calculate cumulative returns
        self.cum_returns = PerformanceMetrics.calculate_cumulative_returns(self.returns)
        print(f"Cumulative returns: {self.cum_returns.tail()}")
