...
class StrategyExplainer:
    ...
    def __init__(self, config=None, config_path='config/config.json'):
        ...
        self.api_key = self.config.get('sentiment').get('llm_api_key', os.environ.get('LLM_API_KEY', ''))
        self.api_url = self.config.get('sentiment').get('llm_api_url', 'https://api.openai.com/v1/chat/completions')
        self.model = self.config.get('sentiment').get('llm_model', 'gpt-3.5-turbo')
        self.max_retries = self.config.get('sentiment').get('max_retries', 3)
        self.retry_delay = self.config.get('sentiment').get('retry_delay', 2)
