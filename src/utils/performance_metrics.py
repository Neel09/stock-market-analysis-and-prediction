...
class PerformanceMetrics:
    ...
    def calculate_returns(prices, positions):
        ...
        # Create a new Series to store the strategy returns
        strategy_returns = pd.Series(0.0, index=price_changes.index)

        # Extract position values
        if isinstance(positions, pd.DataFrame):
            # If positions is a DataFrame, use the first column
            if len(positions.columns) > 0:
                positions_values = positions.iloc[:, 0].values
            else:
                # Empty DataFrame, return zeros
                return pd.Series(0, index=price_changes.index)
        else:
            # If positions is already a Series, use its values
            positions_values = positions.values

        print(f"First 10 position values: {positions_values[:10]}")
        print(f"First 10 price changes: {price_changes.iloc[:10].values}")

        # Calculate strategy returns manually
        for i in range(1, len(price_changes)):  # Start from 1 to skip the first NaN from pct_change
            if i < len(positions_values):
                # Get the position value for the previous day
                if i-1 < len(positions_values):
                    pos_value = positions_values[i-1]
                else:
                    pos_value = 0

                # Ensure pos_value is a number
                try:
                    # Try to convert to float
                    pos_value_float = float(pos_value)

                    # Cap the position value to prevent extreme returns
                    pos_value_float = max(min(pos_value_float, 1.0), -1.0)

                    # Get the price change
                    price_change = price_changes.iloc[i]

                    # Cap the price change to prevent extreme returns
                    ...
                    return_value = price_change * pos_value_float

                    # Cap the return to prevent extreme values
                    return_value = max(min(return_value, 0.1), -0.1)  # Cap at +/- 10%

                    # Store the return
                    strategy_returns.iloc[i] = return_value

                    # Print debug info for the first few iterations
                    if i < 10:
                        print(f"i={i}, pos_value={pos_value}, pos_value_float={pos_value_float}, price_change={price_change}, return={return_value}")
                except (ValueError, TypeError) as e:
                    # If conversion fails, use 0
                    print(f"Error at i={i}: {e}, pos_value={pos_value}, type={type(pos_value)}")
    ...
    def calculate_all_metrics(returns, risk_free_rate=0.0, periods_per_year=252):
        ...
        min_return = -0.9  # -90%
        returns = returns.clip(lower=min_return, upper=max_return)
        ...
        try:
            ...
            try:
                ...
                print(f"Error calculating total_return: {e}")
                total_return = 0
            ...
            try:
                ...
                print(f"Error calculating annualized_return: {e}")
                annualized_return = 0
            ...
            try:
                ...
                print(f"Error calculating annualized_volatility: {e}")
                annualized_volatility = 0
            ...
            try:
                ...
                print(f"Error calculating sharpe_ratio: {e}")
                sharpe_ratio = 0
            ...
            try:
                ...
                print(f"Error calculating sortino_ratio: {e}")
                sortino_ratio = 0
            ...
            try:
                ...
                print(f"Error calculating maximum_drawdown: {e}")
                maximum_drawdown = 0
            ...
            try:
                ...
                print(f"Error calculating calmar_ratio: {e}")
                calmar_ratio = 0
            ...
            try:
                ...
                print(f"Error calculating win_rate: {e}")
                win_rate = 0
            ...
            try:
                ...
                print(f"Error calculating profit_factor: {e}")
                profit_factor = 0

            try:
                expectancy = float(PerformanceMetrics.calculate_expectancy(returns))
                if not np.isfinite(expectancy):
                    expectancy = 0
            except Exception as e:
            ...
            # Final check to ensure all metrics are finite
