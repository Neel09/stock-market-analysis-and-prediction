...
def run_backtest(symbol, period, interval, selected_strategies, strategy_params=None, config=None):
    ...
    try:
        ...
        data = runner.fetch_data(symbol, period, interval)
...
def main():
    ...
    period = st.sidebar.selectbox("Period", options=["1mo", "3mo", "6mo", "1y", "2y", "5y"], index=3, key="period_select")
        ...
        with st.sidebar.expander(f"{strategy_name}", expanded=False):
            ...
            if use_strategy:
                ...
                if strategy_id == "moving_average_crossover":
                    short_window = st.slider("Short Window", min_value=5, max_value=50, value=20, step=1, key=f"short_window_{strategy_id}")
                    long_window = st.slider("Long Window", min_value=20, max_value=200, value=50, step=5, key=f"long_window_{strategy_id}")
                    strategy_params[strategy_id] = {"short_window": short_window, "long_window": long_window}
                
                elif strategy_id == "rsi_strategy":
                    rsi_period = st.slider("RSI Period", min_value=5, max_value=30, value=14, step=1, key=f"rsi_period_{strategy_id}")
                    overbought = st.slider("Overbought Level", min_value=50, max_value=90, value=70, step=5, key=f"overbought_{strategy_id}")
                    oversold = st.slider("Oversold Level", min_value=10, max_value=50, value=30, step=5, key=f"oversold_{strategy_id}")
                    strategy_params[strategy_id] = {"rsi_period": rsi_period, "overbought": overbought, "oversold": oversold}
                
                elif strategy_id == "macd_strategy":
                    fast_period = st.slider("Fast Period", min_value=5, max_value=20, value=12, step=1, key=f"fast_period_{strategy_id}")
                    slow_period = st.slider("Slow Period", min_value=10, max_value=50, value=26, step=1, key=f"slow_period_{strategy_id}")
                    signal_period = st.slider("Signal Period", min_value=5, max_value=20, value=9, step=1, key=f"signal_period_{strategy_id}")
                    strategy_params[strategy_id] = {"fast_period": fast_period, "slow_period": slow_period, "signal_period": signal_period}
                
                elif strategy_id == "bollinger_bands":
                    window = st.slider("Window", min_value=5, max_value=50, value=20, step=1, key=f"window_{strategy_id}")
                    num_std_dev = st.slider("Standard Deviations", min_value=1.0, max_value=4.0, value=2.0, step=0.1, key=f"num_std_{strategy_id}")
                    strategy_params[strategy_id] = {"window": window, "num_std_dev": num_std_dev}
    
    # Advanced options
    with st.sidebar.expander("Advanced Options"):
        initial_capital = st.number_input("Initial Capital", min_value=1000, max_value=1000000, value=100000, step=10000, key="initial_capital")
        commission = st.slider("Commission (%)", min_value=0.0, max_value=1.0, value=0.1, step=0.05, key="commission") / 100
        slippage = st.slider("Slippage (%)", min_value=0.0, max_value=1.0, value=0.1, step=0.05, key="slippage") / 100
        risk_free_rate = st.slider("Risk-Free Rate (%)", min_value=0.0, max_value=10.0, value=2.0, step=0.5, key="risk_free_rate") / 100
    ...
    if run_button:
        ...
        for i, msg in enumerate(["Fetching market data...", "Processing indicators...", "Generating signals...", "Running backtests...", "Calculating metrics..."]):
            ...
            if backtest_results:
                    ...
                    for i, strategy in enumerate(selected_strategies):
                        with cols[i]:
                            if strategy in backtest_results['results']:
                                ...
                                st.markdown(f"<h3 style='text-align: center;'>{strategies[strategy]}</h3>", unsafe_allow_html=True)
                                ...
                                for metric_name, metric_value in metrics.items():
                                    if metric_name in ['total_return', 'sharpe_ratio', 'maximum_drawdown', 'win_rate']:
                                        ...
                                        if metric_name in ['total_return', 'sharpe_ratio'] and float(metric_value.strip('%')) < 0:
                ...
                with tab3:
                    ...
                    if strategy_id and strategy_id in backtest_results['results']:
                        ...
                        no_signals = (positions[positions.columns[0]] == 0).sum()
                        ...
                        # Filter positions
                        filtered_positions = positions[positions[positions.columns[0]].isin(filter_values)]
                        ...
                        if not filtered_positions.empty:
                            ...
                            display_positions = display_positions.rename(columns={display_positions.columns[0]: "Signal Value"})
            ...
            for i, strategy in enumerate(available_strategies):
                with tabs[i]:
                    ...
                    if results:
                        ...
                        with col1:
                            st.image(results['plot_img'], caption=f"{strategies[strategy]} Performance", use_container_width=True)
                        ...
                        with col2:
                            ...
                            for metric_name, metric_value in metrics.items():
                                if metric_name in ['total_return', 'annualized_return', 'sharpe_ratio', 'sortino_ratio', 'maximum_drawdown', 'win_rate', 'profit_factor']:
                                    # Color code based on value
                                    color = "#4CAF50"  # Green for positive
